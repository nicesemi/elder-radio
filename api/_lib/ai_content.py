...
def detect_lead_intent(user_message: str, conversation_history: Optional[list] = None) -> Optional[dict]:
    """
    分析用户消息，检测是否存在购买意向、上门服务等商机需求。
    返回 None 表示无商机；返回 dict 表示检测到需求，包含 lead_type, priority, description。
    """
    ...
    if conversation_history:
        ...
        history_context = "\n".join(
            [f"{'用户' if m.get('role') == 'user' else 'AI'}: {m.get('content', '')}" for m in recent]
        )
...
请分析上述对话中用户是否表达了以下意图（只选其一）：
- 购买意向：想购买设备、语音包、付费功能等
- 上门服务：需要上门安装、维修、教学等
- 咨询：询问价格、功能、服务详情等
- 其他：都不符合
- 无：没有任何商业意图

返回严格的 JSON（不要加任何标记或解释）：
{{"has_lead": true/false, "lead_type": "购买意向/上门服务/咨询/其他", "priority": "高/中/低", "description": "简短描述需求内容"}}
"""
    ...
    try:
        with httpx.Client(timeout=15) as client:
            ...
            if result.get("has_lead"):
                return {
                    "lead_type": result.get("lead_type", "咨询"),
